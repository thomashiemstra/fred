...
class WindowsSerialWrapper:

    def __init__(self, port=None,
                 baudrate=9600,
                 timeout=None):
        self.serial_win_32 = serial.serialwin32.Serial(port, baudrate, timeout=timeout)

    def read(self, size=1):
        return self.serial_win_32.read(size).decode("utf-8")

    def write(self, data):
        return self.serial_win_32.write(data.encode())

    def readline(self):
        return self.serial_win_32.readline().decode("utf-8")

    def flush(self):
        return self.serial_win_32.flush()

    def close(self):
        return self.serial_win_32.close()

    def isOpen(self):
        return self.serial_win_32.is_open()
...
def find_port(baud, timeout):
    ...
    for p in ports:
        ...
        try:
            if is_windows:
                sr = WindowsSerialWrapper(p, baud, timeout=timeout)
            else:
    ...
    def __init__(self, baud=9600, port=None, timeout=2, sr=None):
        ...
        if not sr:
            if not port:
                ...
                if platform.system() == 'Windows':
                    sr = WindowsSerialWrapper(port, baud, timeout=timeout)
                else:
